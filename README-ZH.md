...
- Claude 报告必须包含固定标题。
- strict 任务文件可通过 `windows_scripts/validate_delegate_task.ps1` 预校验。
...
- strict 模式任务文件必须包含 `Goal`、`Allowed Scope`、`Forbidden Actions`、`Acceptance Criteria`、`Verification`、`Report Requirements`。派发前可用 `windows_scripts/validate_delegate_task.ps1` 校验。
